...
async fn test_cache_distinguishes_client_response_profiles() {
    ...
    cache
        .insert(plain.clone(), vec![0x01], Duration::from_secs(300))
        .await;
    cache
        .insert(edns.clone(), vec![0x02], Duration::from_secs(300))
        .await;
    cache
        .insert(forced_dnssec.clone(), vec![0x03], Duration::from_secs(300))
        .await;
