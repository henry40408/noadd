...
Routing and authentication are server-side; page bodies are still vanilla JS web components. No framework, no build step.
...
The shell itself — topbar, both navigation bars, status bar, and any one-shot notice — is rendered by `templates/shell.html`, leaving only `#page-content` for the client to fill. Nothing in `app.js` re-derives what the template already settled: the active navigation item is a class set from the path being answered, and asking the client to work it out again from `location.pathname` would be a second source for one fact. A one-shot notice (the post-setup welcome, the proxy-logout explanation) rides a flash cookie that the rendering response clears, rather than a `?welcome=1` that would survive a refresh, a bookmark and a shared link.
