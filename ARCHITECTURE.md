...
A cache entry holds its response as a `Box<[u8]>`, not a `Vec<u8>` (`src/cache.rs`). A cached response never grows, so the capacity field buys nothing — but the reason this matters is the spare capacity behind it. `Message::to_vec` reserves 512 bytes for every answer it encodes whatever the answer's size, and a typical A response is under 100, so a `Vec` handed straight to `insert` would have the cache holding that difference for the lifetime of the entry. The same applies twice over to the TTL-decremented snapshot `prepare_cached_response` stores on a served entry, which is a second copy of the response; `store_patched_bytes` therefore takes a slice and copies to an exact-sized box rather than adopting the caller's buffer. Shrinking at insert costs one memcpy on the cache-miss path, which has already been to the network.

`tests/cache_memory_bench.rs` is what these claims are measured against: a tracking allocator (an integration test is its own binary and does not link the `mimalloc` that `main.rs` installs) over a fill whose record-type mix follows published resolver traffic. On 10,000 entries the change took a served entry from 1439 to 611 bytes, against a 106-byte average wire payload. The remainder is moka's per-entry bookkeeping and the key, which is where any further work would have to look — not the response bytes. The file's one non-benchmark test asserts the property rather than the number, by comparing what a tight buffer and an over-reserved one cost to cache; only the difference is asserted on, so the threshold encodes nothing about how moka is implemented.
