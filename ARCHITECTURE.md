...
Note that the cache is bounded by **entry count**, not bytes (`DnsCache::new(10_000)` in `src/main.rs`). A DNS response ranges from tens of bytes to several kilobytes, so the resident cost of a full cache varies with the traffic it saw.
