...
Every admin response also carries `X-Frame-Options: DENY` and `Content-Security-Policy: frame-ancestors 'none'` (`security_headers`, `src/headers.rs`), alongside `X-Content-Type-Options: nosniff`, which is the whole clickjacking answer: the admin UI is the only browser-rendered surface noadd serves, nothing in it frames anything (there is not one `<iframe>` in the document), and the two headers say the same thing so browsers that never implemented `frame-ancestors` are still covered. Neither is conditional on TLS the way HSTS is — an appliance reachable over plain HTTP on a LAN is precisely the one an attacker can frame from a page the victim is already browsing. The CSP carries *only* `frame-ancestors`, deliberately: the admin UI is a single HTML file of inline `<script>`/`<style>`, so any broader policy it could satisfy today would need `'unsafe-inline'` (giving up most of what CSP is for), and the nonce alternative means rewriting the document per request, which is incompatible with serving it from `include_dir!` behind a content-hash `ETag`. `nosniff` changes nothing today — every admin response already declares an accurate `Content-Type` — and is there so a future response that gets one wrong fails closed rather than letting a browser sniff an attacker-influenced body into something executable. `Referrer-Policy`, `Permissions-Policy` and the cross-origin isolation headers are absent on purpose rather than by oversight — the admin UI has no outbound links carrying a referrer worth suppressing, requests no powerful features, and needs no cross-origin isolation.
